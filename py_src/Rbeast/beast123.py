...
|> prior <|  prior hyperparameter for the BEAST model 
     ...
     prior.precPriorType    :  a string taking one of 'constant', 'uniform',  'componentwise' (default), and 'orderwise'.
                                ...
                                (4) 'orderwise'}: multiple precision parameters are used to parameterize the model coefficients not just for 
                                 ...
                                 by precValue. In other words, precValue will be inferred by the MCMC, so the fitting result will be 
                                 insensitive to the choice in precValue. 	
     prior.seasonComplexityFactor :  Numeric (defaulted to 0.0); a hyperprior parameter--newly added in Version 1.02--controlling the 
                                  complexity of  the seasonal curve (i.e., the number of seasonal changepoints). A prior of the form 
                                  p(k) ~ exp[lambda*(k+1)]  is placed on the number of seasonal changepoints k, where lambda 
                                  is seasonComplexityFactor (i.e.,s_complexfct:).  Setting lambda = 0 (i.e., seasonComplexityFactor=0)  yields
                                  a non-informative prior p(k) ~ 1.0  where all model dimensions  are equally likely a priori. Users may tune
                                  seasonComplexityFactor or s_complexfct: in the range of [-20, 20]} or an even wider range: negative values
                                  (e.g., lambda = -15.9) favor fewer changepoints (simpler seasonal curves), whereas positive values 
