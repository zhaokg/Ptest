...
<<<<<<< HEAD
				//  Resmapling  GLOBAL_MODEL_PRIOR_FACTOR
				/*				
				F64 sum1 = 0, sum2=0;
				F64 dela = ((*RND.rnd08++)/ 255. - 0.5) * 0.05;
				dela = GLOBAL_MODEL_PRIOR_FACTOR + dela;
				dela = min(dela, 2);
				dela = max(dela, 0);

				for (int i = 0; i < opt->prior.trendMaxKnotNum + 1; i++) {
					sum1 += pow(basis->priorNmodelsPerNseg[i], 1-GLOBAL_MODEL_PRIOR_FACTOR);
					sum2 += pow(basis->priorNmodelsPerNseg[i], 1 - dela);
				}

				F64 ratio1 = pow(1.0/basis->priorNmodelsPerNseg[basis->nKnot + 1-1],dela- GLOBAL_MODEL_PRIOR_FACTOR);

				if ((*RND.rnd08++) / 255.0 < ratio1 * (sum1 / sum2)) {
					GLOBAL_MODEL_PRIOR_FACTOR = dela;
				}
				r_printf("new a: %f %f\n", GLOBAL_MODEL_PRIOR_FACTOR, ratio1* (sum1 / sum2));
			    */
